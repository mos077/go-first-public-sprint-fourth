...
func distance(action int) float64 {
	resDistance := float64(action) * lenStep / mInKm //формула для вычисления дистанции
	return resDistance
}
...
func meanSpeed(action int, duration float64) float64 {
	if duration == 0 {
		return 0
	} // сделали проверку , если время нулевое
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	caloriesRun := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH) // формула для вычисления каллорий при беге
	return caloriesRun
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	caloriesSwim := (speedSwim + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return caloriesSwim
}
